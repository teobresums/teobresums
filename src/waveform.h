...
/**
 * \brief Coefficients of the \f$h^{\rm Newt}_{\ell m}\f$ waveform.
 *
 * \param[in] nu symmetric mass-ratio
 *
 * \return Allocated complex array hlmNewtCoefs of \c size = eobcode_kmax \c
 */
domplex **eob_wav_Newt_coefs (double nu);

/**
 * \brief Newtonian waveform \f$h^{\rm Newt}_{\ell m}(v_\phi)\f$. 
 * 
 * The Newtonian waveform is evaluated on the dynamics using the azimuthal velocity \f$v_\phi=r\Omega\f$. See discussions in
 ...
 * \param[out] hlmNewt      \f$h^{\rm Newt}_{\ell m}(v_\phi)\f$ waveform
 *
 * \return void
 */
 ...
 *
 * Damour, Nagar & Bernuzzi, PRD 87, 084035 (2013) Appendix D
 * <a href="http://arxiv.org/abs/1212.4357">http://arxiv.org/abs/1212.4357</a>
 *
 * \param[in]  nu symmetric mass ratio
 *
 * \return Allocated double array rholmCoefs of \c size = eobcode_kmax x eobcode_wav_rholm_PNcoefs \c
 */
double **eob_wav_rholm_coefs (double nu);

/**
 * \brief Evaluate \f$\rho_{\ell m}(x)\f$ amplitudes at \f$x\f$.
 * 
 ...
 *
 * Take the (2,3) Padee of the 5PN Taylor expansion 
 ...
 * \param[in]  nu symmetric mass ratio
 *
 * \return Allocated double array deltalmCoefs of \c size = eobcode_kmax x eobcode_wav_deltalm_PNcoefs \c
 */
 ...
 * <a href="http://arxiv.org/abs/1212.4357">http://arxiv.org/abs/1212.4357</a>
 *
 * \warning The sym mass-ratio is obtained with a workaround.
 *
 * \param[in]  Heob         Hamiltonian
 * \param[in]  Omega        orbital frequency
 * \param[in]  lmk          multipolar indexes 
 ...
 *
 * Return the six multipolar coefficients 
 * \f$a^{(\ell m)}_i\f$ and \f$b^{(\ell m)}_i\f$ \f$i=1...3\f$
 ...
 * \param[in]  nu symmetric mass-ratio
 *
 * \return Allocated double array NQCab \f$(a^{(\ell m)}_1,a^{(\ell m)}_2,a^{(\ell m)}_3,b^{(\ell m)}_1,b^{(\ell m)}_2,b^{(\ell m)}_3)\f$ 
 * of \c size = eobcode_kmax x eobcode_NQC \c 
 */
 ...
 * \brief Next-to-quasi-circular correction to the waveform.
  * 
  * Calculate
  * \f$\hat{h}^{\rm NQC}_{\ell m} = (1 + a^{(\ell m)}_i n^i)\exp[i (a^{(\ell m)}_i n^{i+3})]\f$
  * 
  ...
  * \param[out] hlmNQC   \f$\hat{h}^{\rm NQC}_{\ell m}\f$
  * 
  * \return void
  */
...
/**
 * \brief Source factor \f$S^{\epsilon}_{\rm eff}\f$ of the resummed waveform.
 *
 * Damour, Iyer & Nagar, PRD 79, 064004 (2009)
 * <a href="http://arxiv.org/abs/0811.2069">http://arxiv.org/abs/0811.2069</a>
 *
...
/**
 * \brief Evaluate \f$S^{\epsilon}_{\rm eff}\f$ on time series.
 *
 * \param[in]  Heff  Effective Hamiltonian \f$\hat{H}_{\rm eff}\f$
 * \param[in]  jhat  \f$p_\phi/(r_\omega v_\phi)\f$
 * \param[in]  lmk   multipolar indexes
