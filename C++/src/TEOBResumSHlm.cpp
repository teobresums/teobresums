...
vector<double> s_flm(double x, void *params){
  ...
  /* 
     ...
     are integrated by the 4PN and 5PN test-particle terms, with the higher
     modes obtained by Fujita & Iyer.  
     We added spin-spin term at NLO *only* for the tidal sector
  */
  ...
  ************************************************************************************************************/
  double cSO_lo    = (-0.5*a0 - a12X12/6.);
  double cSO_nlo   = (-52./63.-19./504.*nu)*a0 - (50./63.+209./504.*nu)*a12X12;
  double cSO_nnlo  = (32873./21168. + 477563./42336.*nu + 147421./84672.*nu*nu)*a0 - (23687./63504. - 171791./127008.*nu + 50803./254016.*nu*nu)*a12X12;
  ...
  if (tidal_flag==true)
    ...
    {
      cSS_lo  = 0.5*a0*a0;
      cSS_nlo = 0.;//1./504.*(2.*(19. - 70.*nu)*a12*a12 + (-302. + 243.*nu)*a0*a0 + 442.*X12*a0*a12);
    }
  ...
  *************************************************************************/
  ...
  rho32S = (a0-a12X12)/(3.*(1.-3.*nu))*v; // + c32S0_nlo;
  rho44S = (-19./30.*a0 -  (1.-21.*nu)/(30.-90.*nu)*a12X12)*v3;
  rho42S = ( -1./30.*a0 - (19.-39.*nu)/(30.-90.*nu)*a12X12)*v3;
  ...
  *************************************************************************/
  ...
  if (tidal_flag==true)
    ...
    {
      c21SS_lo  = 0.;//1./8.*(-27.*(a1*a1 - a2*a2) + (3.*a1*a1 + 10.*a1*a2 + 3.*a2*a2)*X12);
      c33SS_lo  = 0.;//3./2.*a0*a0*X12;
      c31SS_lo  = 0.;//-4.*(a1*a1 - a2*a2) + 3./2.*a0*a0*X12;
    }
