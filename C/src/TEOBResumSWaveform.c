 ...
 *   @param[out] flm  : resummed amplitudes
*/
void eob_wav_flm_Kerr(double x, double UNUSED(nu), double *rholm, double *flm)
{
  /** Coefficients */
 ...
 */
void eob_wav_flm_s_SSLO(double x, double nu, double X1, double X2, double UNUSED(chi1), double UNUSED(chi2), double a1, double a2, double C_Q1, double C_Q2, int usetidal,
 ...
 *   @param[out] flm: residual amplitude corrections
*/
void eob_wav_flm_s_SSNLO(double x, double nu, double X1, double X2, double UNUSED(chi1), double UNUSED(chi2), double a1, double a2, double C_Q1, double C_Q2, int usetidal,
			 double *rholm, double *flm)
{
 ...
 *   @param[out] flm: residual amplitude corrections
 */
void eob_wav_flm_s_HM(double x, double nu, double X1, double X2, double UNUSED(chi1), double UNUSED(chi2), double a1, double a2,
 ...
 *   @param[out] flm: residual amplitude corrections
*/
void eob_wav_flm_s_Kerr(double x, double nu, double UNUSED(X1), double UNUSED(X2), double UNUSED(chi1), double UNUSED(chi2), double a1, double UNUSED(a2),
		      double UNUSED(C_Q1), double UNUSED(C_Q2), int UNUSED(usetidal), double *rholm, double *flm)
{
  /** Orbital part */
  eob_wav_flm(x,nu, rholm, flm);
...
void eob_wav_hlmNQC_find_a1a2a3(Dynamics *dyn, Waveform_lm *h, Waveform_lm *hnqc)
{
  ...
  if (EOBPars->use_flm == USEFLM_HM) {
    ...
    eob_nqc_point_HM(max_A, max_dA, max_omg, max_domg);
    
    /* 21 fitted directly at tpeak_22*/
    eob_nqc_point_HM_peak22(max_A, max_dA, max_omg, max_domg);
    ...
    eob_nqc_point(&A_tmp, &dA_tmp, &omg_tmp, &domg_tmp);
...
/**
 * Function: eob_set_nqc_basis_HM
 * ------------------------------
 *   Compute the NQC basis functions n1,n2,n4,n5 and 
 *   their time derivatives
 * 
 *   @param[in]  kmask : mask for the modes
 *   @param[in]  t     : time
 *   @param[in]  r     : radial separation
 *   @param[in]  w     : Omega
 *   @param[in]  pr_star : pr_star
 *   @param[in]  ddotr : second time derivative of r
 *   @param[in]  size  : size of the arrays
 *   @param[out] n1,n2,n4,n5 : basis functions
 *   @param[out] d_n4,d_n5 : first time derivatives of n4,n5
 *   @param[out] d2_n4,d2_n5 : second time derivatives of n4,n5
 * 
*/
void eob_set_nqc_basis_HM(int *kmask, double *t, double *r, double *w, double *pr_star, double * ddotr, int size, 
                          double **n1, double **n2, double **n4, double **n5,
                          double **d_n4, double ** d_n5, double **d2_n4, double **d2_n5)
{
  ...
  // Compute the part common to all modes
  for (int k=0; k<KMAX; k++) { 
    if(kmask[k]){ 
      for (int j=0; j<size; j++) {
        pr_star2 = SQ(pr_star[j]);
  ...
  // For some HM, modify n2, n5 or both
  if (EOBPars->use_flm == USEFLM_HM) {
    for (int j=0; j<size; j++) {
      /* l=2,m=1 */
      if(kmask[0]) {
  ...
  /** Derivatives of n4, n5 for the phase */
  if(d_n4 != NULL && d_n5 !=NULL && d2_n4 != NULL && d2_n5 !=NULL){
    for (int k=0; k<KMAX; k++) {
      if(kmask[k]){   
        D0(n4[k],dt,size, d_n4[k]);
        D0(n5[k],dt,size, d_n5[k]);
...
/**
 * Function: eob_solve_system_nqc_HM
 * ---------------------------------
 *   Solve the linear system for the NQC coefficients
 *   
 *   @param[in]  h   : multipolar waveform
 *   @param[in]  max_A   : max amplitude
 *   @param[in]  max_dA  : amplitude derivative at the peak
 *   @param[in]  max_omg : omega at the peak
 *   @param[in]  max_domg: omega derivative at the peak
 *   @param[in]  d_n4,d2_n4,n1,n2,n4,n5,d_n5,d2_n5    : basis functions
 *   @param[in]  j_NQC : index of NQC matching point for each mode
 *   @param[in]  jmax  : index such that t[jmax] <= tNQC22
 *   @param[out] ai    : NQC coefficients for the amplitude
 *   @param[out] bi    : NQC coefficients for the phase
*/  
void eob_solve_system_nqc_HM(Waveform_lm *h, 
                          double max_A[KMAX], double max_dA[KMAX], double max_omg[KMAX], double max_domg[KMAX],                                                                                // Fit vals
                          double *d_n4[KMAX], double *d2_n4[KMAX], double *n1[KMAX], double *n2[KMAX], double *n4[KMAX], double *n5[KMAX], double *d_n5[KMAX], double *d2_n5[KMAX],            // basis functions
                          int j_NQC[KMAX], int jmax,                                              
                          double ai[KMAX][2], double bi[KMAX][2])                                                                                                                               // output
{
...
#if (DEBUG)
  FILE* fp_dbg = fopen("nqc_amp_func.txt", "w");
  for (int j=0; j<size; j++) {
    fprintf(fp_dbg, "%e\t%e\t%e\n", t[j], p1tmp[1][j], p2tmp[1][j]);
  }
  fclose(fp_dbg);  
  ...
  if (EOBPars->output_nqc_coefs) {
    ...
    for (int k=0; k<KMAX; k++) {
      if(h->kmask[k]){
        fprintf(fp, "%d %d %d %e %e %e %e\n", k, LINDEX[k], MINDEX[k], ai[k][0], ai[k][1], bi[k][0], bi[k][1]);
      }
    }  
    fclose(fp);  
  }
				       ...
				       Dynamics *dyn, Waveform_lm *hlm)
{
  ...
  double *n1[KMAX],*n2[KMAX],*n3[KMAX],*n4[KMAX],*n5[KMAX],
         *d_n4[KMAX],*d_n5[KMAX],*d2_n4[KMAX],*d2_n5[KMAX];
  
  for (int k=0; k<KMAX; k++) {
    if(hlm_mrg->kmask[k]){
      n1[k]    = (double*) calloc (size,sizeof(double));      n2[k]    = (double*) calloc (size,sizeof(double));
      n4[k]    = (double*) calloc (size,sizeof(double));      n5[k]    = (double*) calloc (size,sizeof(double));
      d_n4[k]  = (double*) calloc (size,sizeof(double));      d_n5[k]  = (double*) calloc (size,sizeof(double));      
      d2_n4[k] = (double*) calloc (size,sizeof(double));      d2_n5[k] = (double*) calloc (size,sizeof(double));
    }
  }

  /* NR fits */
  double max_A[KMAX]    = {0.};  double max_dA[KMAX]   = {0.};
  double max_omg[KMAX]  = {0.};  double max_domg[KMAX] = {0.};
  ...
  eob_set_nqc_basis_HM(hlm_mrg->kmask, t, r, w, pr_star, ddotr, size, n1, n2, n4, n5, d_n4, d_n5, d2_n4, d2_n5);

  /* Find max Omg */
  int Omgmax_index = 0;
  double Omg_max   = Omg_orb[0];
  find_max_array(Omg_orb, size, &Omg_max, &Omgmax_index);
  /* Time */
  double DeltaT_nqc = eob_nqc_timeshift(nu, chi1);
  double tNQC = t[Omgmax_index] - DeltaT_nqc;
  if (VERBOSE) {
    printf("NQC info:\n");
    PRFORMd("DeltaT_tNQC",DeltaT_nqc);
    PRFORMd("tNQC[bare]",tNQC);
  ...
  */
  ...
  /* Solve the linear systems */
  eob_solve_system_nqc_HM(hlm_mrg, max_A, max_dA, max_omg, max_domg, d_n4, d2_n4, n1, n2, n4, n5, d_n5, d2_n5, j_NQC, jmax, ai, bi);
...
void eob_wav_hlmNQC_find_a1a2a3_mrg_22_BNS(Dynamics *dyn_mrg, Waveform_lm *hlm_mrg, Waveform_lm *hnqc,
				       Dynamics *dyn, Waveform_lm *hlm)
{
#define KMAX_22 (2) // 0, 1 -> (2,1), (2,2) // k=0 unused.
  ...
  /** NR fits */
  double pl_hold;
  Breschi22_nr_mrg_fits_bns(&A_tmp, &dA_tmp, &pl_hold, &omg_tmp, &domg_tmp);
 ...
 *   @param[out] hlmnqc : NQC part of the waveform
 */
void eob_wav_hlmNQC(double  r, double  prstar, double  Omega, double  ddotr, NQCcoefs *nqc, 
		    Waveform_lm_t *hlmnqc)
{      
...
void eob_wav_ringdown_HM(Dynamics *dyn, Waveform_lm *hlm)
{
  ...
  */
  ...
  QNMHybridFitCab_HM_Pompili23(nu, chi1, chi2, Mbh,
                                a1, a2, a3, a4, b1, b2, b3, b4,
                                sigma[0], sigma[1]
                              );
  /** Define a time vector for each multipole, scale by mass
...
/**
  ...
  *   @param[out] flm     : resummed amplitudes
*/
void eob_wav_flm_s_old(double x, double nu, double X1, double X2, double UNUSED(chi1), double UNUSED(chi2), double a1, double a2, double C_Q1, double C_Q2, int usetidal,
		       double *rholm, double *flm)
{
...
/**
  ...
  *
*/
void twist_hlm_TD(Dynamics *dyn, Waveform_lm *hlm, DynamicsSpin *spin,
...
{  
    nu; distance; // dummy call to avoid unused warning
#ifdef _OPENMP
...
/**
  ...
  *
*/
void compute_hpc_FD(WaveformFD_lm *hflm, double UNUSED(nu), double M, double UNUSED(distance), double amplitude_prefactor, double phi, double iota, WaveformFD *hpc)
{  
#ifdef _OPENMP
				       ...
				       Dynamics *dyn, Waveform_lm *hlm)
{
  ...
  /* 22, 31, 33, 41 and 55 fitted directly + 44 dA */
  eob_nqc_point_BHNS_HM(max_A, max_dA, max_omg, max_domg, kt2);
