...
#define KMAX_22 (2) // 0, 1 -> (2,1), (2,2) // k=0 unused.
  ...
  if ((ecc != 0.) || (r_hyp != 0.)) {
    ...
    */
    ...
    double fact[KMAX];
    // Correction needed to have sigmoid = 1 when attaching ringdown
    fact[k22] = 1. + exp(-alpha*(t[jmax] - t0));
    
    for (int j=0; j<size; j++) {
      sigmoid[j] = 1./(1. + exp(-alpha*(t[j] - t0)));
      
      n1[k22][j] = n1[k22][j]*sigmoid[j]*fact[k22];
      n2[k22][j] = n2[k22][j]*sigmoid[j]*fact[k22];
      n4[k22][j] = n4[k22][j]*sigmoid[j]*fact[k22];
      n5[k22][j] = n5[k22][j]*sigmoid[j]*fact[k22];
    }
