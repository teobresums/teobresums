...
{
  double nu2 = nu*nu;
  return 277.6835807911776*nu2 -437.8200772342428*nu -22.582437062389737;
}
 ...
 *  -----------------------------------
 *   New fit for c3 obtained with IMPQC settings in Dali'
 *   Note: c3 = 0 with tides
 *
 *   @param[in] nu: symmetric mass ratio
 *   @param[in] a1: spin of body 1
...
double eob_c3_fit_ecc_P33_impqc(double nu, double a1, double a2)
{
  ...
  /* Equal mass part */
  const double p0 = 38.316272147907846;
  const double n1 = 0.28836631381609557;
  const double n2 = -0.8872832041219025;
  const double n3 = 0.3314581165198756;
  const double n4 = -0.07847346417511875;
  const double d1 = 1.250251693391218;
  const double c3_eq = p0*(1. + n1*a0 + n2*a02 + n3*a03 + n4*a04)/(1.+ d1*a0);

  /* Unequal mass, unequal-spin part */
  const double c1 =  19.372770269785683;
  const double c2 =  12.601043854959949;
  const double c3 = -21.13404034435283;
  const double c4 = -56.38151796961188;
  const double c5 =  37.5701494879252;
  const double c6 = -151.2719758819389;
...
/** 
 *  Function: eob_nqc_point_postpeak_d2
 *  -----------------------------------
 *   This function computes the NQC functioning points by evaluating 
			    ...
			    double *A_tmp, double *dA_tmp, double *d2A_tmp, double *omg_tmp, double *domg_tmp, double *d2omg_tmp)
{
  ...
  tau  = 2./Mbh;
  Mbh2 = SQ(Mbh);
  double expt = exp(-alpha1*tau);
  double tanht = tanh(c2A*tau + c3A);
  
  *A_tmp = expt*(c1A*tanht + c4A);
  
  double fact = cosh(c3A+ c2A*tau);
  dA_tmp1     = c1A*c2A*expt/(fact*fact);
  dA_tmp2     = -alpha1*expt*(c4A + c1A*tanh(c3A+ c2A*tau));
  *dA_tmp     = (dA_tmp1 + dA_tmp2)/Mbh;
  *d2A_tmp    = (alpha1*alpha1*(expt*(c1A*tanht + c4A)) - 2.0*expt*c1A*c2A*(alpha1 + c2A*tanht)/(fact*fact))/Mbh2;
  ...
  *omg_tmp = (omg_tmp1/omg_tmp2)/Mbh + omega1/Mbh;
  ...
  domg_tmp_d2 = 1 + c4phi*x2 + c3phi*x;
  *domg_tmp   = -(domg_tmp_n1/domg_tmp_d1 - domg_tmp_n2/domg_tmp_d2)/Mbh2;
  *d2omg_tmp  = -c1phi*c2phi*c2phi*c2phi*x*(1.0 - c4phi*x2)*(8.0*c4phi*x*(1.0 + c4phi*x2) + c3phi*(1.0 + x2*(10.0*c4phi - c3phi*c3phi + c4phi*c4phi*x2)))/domg_tmp_d1/domg_tmp_d2/Mbh2/Mbh;
}
/** @} */ // end of nqcpoint
