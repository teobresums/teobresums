...
/**
 *  Function: eob_a6c_fit_LMR
 *  -------------------------
 *   a6c NR-calibrated fit for the LMR model (A_5PNlogP33_newlogs +
 *   D_5PNP32_newlogs + Q_5PNfull, USEFLM_22PN + HFLX_LMR).  a6c is the
 *   NR-calibration coefficient of the A_5PNlogP33_newlogs potential.
 *   Quadratic in nu, calibrated by minimising the EOB/NR (2,2) merger-phase
 *   residual vs SXS (q = 1, 2, 4.5, 8, 20); see
 *   teob-lmr/PythonScripts/tune_a6c_LMR.py and Mathematica/a6c_ALMR.nb
 *   (FindFit a*nu^2 + b*nu + c, fitsq4).
 *    @param[in] nu: symmetric mass ratio
 *    @return  a6c
 */
double eob_a6c_fit_LMR(double nu)
{
  return 42.7471 + nu*(-472.265 + nu*589.107);
}
...
/**
 *  Function: eob_c3_fit_LMR
 *  ------------------------
 *   cN3LO (c3) NR-calibrated fit for the LMR model (A_5PNlogP33_newlogs +
 *   D_5PNP32_newlogs + Q_5PNfull, USEFLM_22PN + HFLX_LMR).
 *   Two contributions c3 = c3_eq + c3_neq (Eqs. (37)-(38b) of arXiv:2407.04762):
 *   equal-mass/spin Pade in a0 + unequal-mass terms.  a1 = X1*chi1, a2 = X2*chi2
 *   (mass-weighted); a0 = a1+a2, a12 = a1-a2.  Calibrated by minimising the
 *   (2,2) mismatch vs spinning SXS; generated by PythonScripts/tune_c3_LMR.py.
 *    FIXME: mismatches are not optimal; will change functional form.
 *    @param[in] nu: symmetric mass ratio
 *    @param[in] a1: X1*chi1
 *    @param[in] a2: X2*chi2
 *    @return  cN3LO
...
double eob_c3_fit_LMR(double nu, double a1, double a2)
{
  const double nu2 = nu*nu;
  const double X12 = sqrt(1. - 4.*nu);
  const double a0  = a1 + a2;          /* atilde0  */
  const double a02 = a0*a0, a03 = a02*a0, a04 = a03*a0;
  const double a12 = a1 - a2;          /* atilde12 */

  /* equal-mass, equal-spin (Eq. 38a) */
  const double p0 = 34.743879;
  const double n1 = -1.195123;
  const double n2 = 0.408300;
  const double n3 = -1.012583;
  const double n4 = 0.840990;
  const double d1 = -0.817638;
  const double c3_eq = p0*(1. + n1*a0 + n2*a02 + n3*a03 + n4*a04)/(1. + d1*a0);

  /* unequal-mass corrections (Eq. 38b) */
  const double p1 = -18.333449;
  const double p2 = 16.244027;
  const double p3 = 0.818008;
  const double p4 = -27.580362;
  const double p5 = 9.297368;
  const double p6 = -193.150031;
  const double c3_neq = (p1*a0 + p2*a02 + p3*a03)*X12 + p4*a0*nu*X12
                      + p5*a12*X12 + p6*a12*a12*nu2;
