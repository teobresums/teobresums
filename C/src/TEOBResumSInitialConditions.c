...
double eob_dyn_circ_j0(double r0, Dynamics *dyn)
{
  ...
  if(usespins) {
    ...
    eob_dyn_s_get_rc(r0, nu, a1, a2, aK2, C_Q1, C_Q2, C_Oct1, C_Oct2, C_Hex1, C_Hex2, usetidal, &rc, &drc_dr, &pl_hold, &pl_hold);
...
void eob_dyn_ic_ecc_ma(double r0_kepl, Dynamics *dyn, double y_init[])
{
  ...
  if(usespins) {
    ...
    eob_dyn_s_get_rc(rma, nu, a1, a2, aK2, C_Q1, C_Q2, C_Oct1, C_Oct2, C_Hex1, C_Hex2, usetidal, &rcma, &pl_hold, &pl_hold, &pl_hold);
  } else {
  ...
  /* Energy */
  eob_ham_s(nu, rma, rcma, 0., 0., 0., j0, pr0, S, Sstar, chi1, chi2, X1, X2, aK2, c3, Ama, 0., 0., Qma, 0., 0., 0., 0.,
...
int eob_dyn_rpr(const gsl_vector *x, void * params, gsl_vector *f)
{
  ...
  if(usespins) {
    ...
    eob_dyn_s_GS(r1, rc1, 0.0, 0.0, 0., aK2, 0.0, 0.0, nu, chi1, chi2, X1, X2, c3, ggm1);
    G1     = ggm1[2]*S + ggm1[3]*Sstar;    // tildeG = GS*S+GSs*Ss

    eob_metric_s(r2, 0., dyn, &A2, &pl_hold, &pl_hold, &pl_hold, &pl_hold, &pl_hold, &pl_hold, &pl_hold, &pl_hold, &pl_hold, &pl_hold, &pl_hold, &pl_hold, &pl_hold, &pl_hold);
    eob_dyn_s_get_rc(r2, nu, a1, a2, aK2, C_Q1, C_Q2, C_Oct1, C_Oct2, C_Hex1, C_Hex2, usetidal, &rc2, &pl_hold, &pl_hold, &pl_hold);
    
    eob_dyn_s_GS(r2, rc2, 0.0, 0.0, 0., aK2, 0.0, 0.0, nu, chi1, chi2, X1, X2, c3, ggm2);
    G2     = ggm2[2]*S + ggm2[3]*Sstar;
