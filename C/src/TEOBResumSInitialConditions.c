...
#define N (6)
  ...
  for (int i = 0; i < 2*N; i++) {
    ...
    /** Compute metric  */
    eob_metric(r[i], dyn, &A, &B, &dA[i], &d2A, &dB, &pl_hold);
...
#define N (6)
  ...
  double A[2*N],B[2*N],d2A[2*N],dB, sqrtAbyB, pl_hold;
  ...
  for (i = 0; i < 2*N; i++) {
    ...
    /** Compute metric  */
    eob_metric_s(r[i], dyn, &A[i],&B[i],&dA[i],&d2A[i],&dB,&pl_hold);
  ...
  for (i = 0; i < 2*N; i++) {
    ...
    Fphi[i]    = eob_flx_Flux_s(x, Omg, r_omg, H0, Heff0, jhat, r[i], 0.0, 0.0, dyn);
...
void eob_dyn_ic_ecc(double r0, Dynamics *dyn, double y_init[])
{
  ...
  if(usespins) {
    eob_metric_s(r1, dyn, &A1, &pl_hold, &pl_hold, &pl_hold, &pl_hold);
    eob_dyn_s_get_rc(r1, nu, a1, a2, aK2, C_Q1, C_Q2, C_Oct1, C_Oct2, C_Hex1, C_Hex2, usetidal, &rc1, &pl_hold, &pl_hold);
    B1 = A1/SQ(rc1);
    eob_dyn_s_GS(r1, rc1, 0.0, aK2, 0.0, 0.0, nu, chi1, chi2, X1, X2, c3, ggm1);
    ...
    B2 = A2/SQ(rc2);
    eob_dyn_s_GS(r2, rc2, 0.0, aK2, 0.0, 0.0, nu, chi1, chi2, X1, X2, c3, ggm2);
...
double eob_dyn_DHeff0(double x, void *params)
{
  ...
  double ggm0[26];
  eob_dyn_s_GS(rorb, rc, drc_dr, 0.0, ak2, 0.0, x, nu, chi1, chi2, X1, X2, c3, ggm0);
...
double eob_dyn_Omegaorb0(double r, void *params)
{
  ...
  if(usespins) {
    ...
    eob_metric(r ,dyn, &A, &B, &dA, &pl_hold, &pl_hold, &pl_hold);
