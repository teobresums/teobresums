...
/**
 *  Function : eob_metric_Atidal_electric_NNLO
 *  ---------------------------------
 *   Tidal potential, electric part NNLO, Bernuzzi+ 1205.3403
 *   
 ...
 *  Function : eob_metric_Atidal_electric_NNLO
 *  ---------------------------------
 *   Tidal potential, electric part N3LO, Schulze+
 *   
 *   f-mode resonance model from Hinderer&Steinhoff can be added
 *
 *   @param[in]  r     : radial separation
...
void eob_metric_Atidal_electric_N3LO(double r, Dynamics *dyn, double *AT, double *dAT, double *d2AT)
{
  ...
  double bar_alph2_1 = EOBPars->bar_alph2_1;
  double bar_alph2_2 = EOBPars->bar_alph2_2;
  double bar_alph2_3 = EOBPars->bar_alph2_3;
  double bar_alph3_1 = EOBPars->bar_alph3_1;
  double bar_alph3_2 = EOBPars->bar_alph3_2;
  ...
  double kapB4_u = kapB4;
  double kapT2_u=0, kapT3_u=0, kapT4_u=0;
  double bar_alph2_1_u=0, bar_alph2_2_u=0, bar_alph2_3_u=0, bar_alph3_1_u=0, bar_alph3_2_u=0;
  ...
  if (EOBPars->use_tidal_fmode_model) {
    ...
    */
    ...
    const double cA22 = (3.+XA/8.+ 337./28.*XA*XA);
    const double cB22 = (3.+XB/8.+ 337./28.*XB*XB);
    const double cA23 = (9. + XA*(1905*Pi*Pi/256. - 3487./16.) + XA*XA*(1943967./9800. - 1905*Pi*Pi/256.) + XA*XA*XA*937./56. + XA*XA*XA*XA*10);
    const double cB23 = (9. + XB*(1905*Pi*Pi/256. - 3487./16.) + XB*XB*(1943967./9800. - 1905*Pi*Pi/256.) + XB*XB*XB*937./56. + XB*XB*XB*XB*10);
    const double cA31 = (-2.+15./2.*XA);
    const double cB31 = (-2.+15./2.*XB);
    const double cA32 = (8./3.-311./24.*XA+110./3.*XA*XA);
    const double cB32 = (8./3.-311./24.*XB+110./3.*XB*XB);
    ...
    bar_alph2_1 = bar_alph( cA21, kapA2, cB21, kapB2, kapT2 );
    bar_alph2_2 = bar_alph( cA22, kapA2, cB22, kapB2, kapT2 );
    bar_alph2_3 = bar_alph( cA23, kapA2, cB23, kapB2, kapT2 );
    bar_alph3_1 = bar_alph( cA31, kapA3, cB31, kapB3, kapT3 );
    bar_alph3_2 = bar_alph( cA32, kapA3, cB32, kapB3, kapT3 );
    ...
    bar_alph2_2_u =
      ...
      + bar_alph( cA22, kapA2_u, cB22, kapB2_u, kapT2 );
    bar_alph2_3_u =
      - bar_alph2_3 * kapT2_u/kapT2
      + bar_alph( cA23, kapA2_u, cB23, kapB2_u, kapT2 );
    bar_alph3_1_u =
      - bar_alph3_1 * kapT3_u/kapT3
      + bar_alph( cA31, kapA3_u, cB31, kapB3_u, kapT3 );
    bar_alph3_2_u =
  ...
  if (EOBPars->use_tidal_fmode_model) {
    ...
    dA_u -= kapT2_u*u6*(1.+ bar_alph2_1*u + bar_alph2_2*u2 + bar_alph2_3*u3);
    dA_u -= kapT2*u6*(bar_alph2_1_u*u + bar_alph2_2_u*u2 + bar_alph2_3_u*u3);
  }    
...
/**
 *  Function : eob_metric_Atidal_electric_TEOBResum
 *  ---------------
 *   Tidal potential, TEOBResum, Bini&Damour, 1409.6933, Bernuzzi+ 1412.4553   
 *   f-mode resonance model from Hinderer&Steinhoff can be added
 *
 *   @param[in]  r     : radial separation
 *   @param[in]  dyn   : EOB dynamics 
 ...
 *   The electric part is computed calling eob_metric_Atidal_electric, set as
 *   a function pointer depending on the chosen tidal model.
 *   For the moment, magneic contributions are added here using if-else statements.
 *   In the future, one might want to modularize this further & have separate functions
 *   also for those
 *
 *   @param[in]  r     : radial separation
 *   @param[in]  dyn   : EOB dynamics 
...
void eob_metric_Atidal(double r, Dynamics *dyn, double *AT, double *dAT, double *d2AT)
{
  ...
  double A=0., dA_u=0., d2A_u=0., dA=0., d2A=0.;
  eob_metric_Atidal_electric(r, dyn, &A, &dA_u, &d2A_u);
 ...
 *  ---------------
 *   Tidal B potential
 *   Vines, Flanagan 1PN term electric part
 *
 *   @param[in]  r     : radial separation
 *   @param[in]  dyn   : EOB dynamics 
...
void eob_metric_Btidal_electric_3PN(double r, Dynamics *dyn, double *BT, double *dBT, double *d2BT)
{
  ...
  const double cnu = (8. - 15.*nu);
  const double d6_e = kapT2*cnu; 
  const double tmp_u = kapT2_u*cnu;

  const double d7_e = kapA2 * (39. - XA * 129. / 8. + XA2 * 141. / 4. + XA3 * 105 / 2) + 
                      kapB2 * (39. - XB * 129. / 8. + XB2 * 141. / 4. + XB3 * 105 / 2);
 ...
 *  ---------------
 *   Tidal B potential, puts together electric and magnetic contributions.
 *   The electric part is computed calling eob_metric_Btidal_electric, set as
 *   a function pointer depending on the chosen tidal model.
 *
 *   @param[in]  r     : radial separation
 *   @param[in]  dyn   : EOB dynamics 
...
{
  double B_elec=0., dB_elec=0., d2B_elec=0.;
  double B_mag=0., dB_mag=0., d2B_mag=0.;
  ...
  *BT   = B_elec + B_mag;
  *dBT  = dB_elec + dB_mag;
  if (d2BT != NULL) *d2BT = d2B_elec + d2B_mag;
}
                     ...
                     double *d3QT_dr2dprstar, double *d3QT_drdprstar2, double *d3QT_dprstar3)
{
  ...
  Q = prstar4 * (q46_e * u6 + q47_e * u7) + prstar6 * q66_e * u6;
  dQ_u = prstar4 * (6. * q46_e * u5 + 7. * q47_e * u6) + 6. * prstar6 * q66_e * u5;
  d2Q_u = prstar4 * (30. * q46_e * u4 + 42. * q47_e * u5) + 30. * prstar6 * q66_e * u4;

  dQ_prstar = 4. * prstar3 * (q46_e * u6 + q47_e * u7) + 6. * prstar5 * q66_e * u6;
  d2Q_prstar = 12. * prstar2 * (q46_e * u6 + q47_e * u7) + 30. * prstar4 * q66_e * u6;
  d3Q_prstar = 24. * prstar * (q46_e * u6 + q47_e * u7) + 120. * prstar3 * q66_e * u6;
...
void eob_metric_Qtidal(double r, Dynamics *dyn, double prstar, double nu, double *QT, double *dQT_du, double *dQT_dprstar,
                       ...
                       double *d3QT_dr2dprstar, double *d3QT_drdprstar2, double *d3QT_dprstar3)
{
  double Q_elec=0., dQ_du_elec=0., d2Q_du2_elec=0.;
...
/** 
 *  Function: zero_tidal_potential
 * ----------------------
 *   Sets all potentials and derivatives to zero
 *   Used when tidal effects are switched off
 *  @param[in]  r     : radial separation
 *  @param[in]  dyn   : EOB dynamics
 *  @param[out] PT    : tidal potential evaluated at r  
 *  @param[out] dPT   : tidal dP/du,   with u=1/r  
 *  @param[out] d2PT  : tidal d2P/du2, with u=1/r 
 */
                ...
                double *d3Q_dr2dprstar, double *d3Q_drdprstar2, double *d3Q_dprstar3)
{
  ...
  if (EOBPars->use_tidal) {
    ...
    eob_metric_Qtidal(r, dyn, prstar, nu, &QT, &dQT_du, &d2QT_du2, &dQT_dprstar, &d2QT_dprstar2, &d3QT_dprstar3, &d2QT_drdprstar, &d3QT_dr2dprstar, &d3QT_drdprstar2);
    Qt += QT;
    dQt_du += dQT_du;
    d2Qt_du2 += d2QT_du2;
    ...
    d2Qt_drdprstar += d2QT_drdprstar;
    d3Qt_drdprstar2 += d3QT_drdprstar2;
    d3Qt_dr2dprstar += d3QT_dr2dprstar;
  }
  ...
  *Q              = Qtmp + Qt;
  *dQ             = -u2*(dQtmp_du + dQt_du); // derivative wrt to r
  *dQ_dprstar     = dQtmp_dprstar + dQt_dprstar;
  *d2Q            = u3*(2.*(dQtmp_du + dQt_du) + u*(d2Qtmp_du2 + d2Qt_du2)); // derivative wrt to r
  *ddQ_drdprstar  = ddQtmp_drdprstar + d2Qt_drdprstar;
  *d2Q_dprstar2   = d2Qtmp_dprstar2 + d2Qt_dprstar2;
  *d3Q_dr2dprstar = d3Qtmp_dr2dprstar + d3Qt_dr2dprstar;
  *d3Q_drdprstar2 = d3Qtmp_drdprstar2 + d3Qt_drdprstar2;
