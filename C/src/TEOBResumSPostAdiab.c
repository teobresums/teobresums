 ...
 /* Parameters for post adiabatic dynamics */
  ...
  if (usespins == MODE_SPINS_GENERIC && EOBPars->project_spins){
    if(EOBPars->spin_interp_domain==0){
      ...
      for(int i=0; i<size;i++){
        ...
        //printf("basic r, omg, chis =%.10f %.10f %.10f %.10f %.10f %.10f\n", r0 - i*dr, Omg_i, chi1_grid[i], chi2_grid[i], SApara, SBpara);
      }
    }
  }
  if (VERBOSE) {
  ...
  for (int i = 0; i < size; i++) {
    ...
    if(usespins){ 
      ...
      eob_dyn_s_GS(dyn->r, rc_vec[i], drc_dr_vec[i], EOBPars->aK2, 0.0, 0.0, nu, chi1, chi2, X1, X2, c3, ggm);
    ...
    */
    ...
    if(usespins) {
      ...
      eob_ham_s(nu, dyn->r, rc_vec[i], drc_dr_vec[i], dyn->pphi, dyn->prstar, EOBPars->S, EOBPars->Sstar, chi1, chi2, X1, X2, EOBPars->aK2, c3, A_vec[i], dA_vec[i],
  ...
  for (int n = 1; n <= Npa; n++) {
    ...
    for (int i = 0; i < size; i++) {
      ...
      if (usespins == MODE_SPINS_GENERIC && EOBPars->project_spins){
        ...
        eob_dyn_s_get_rc(dyn->r, nu, EOBPars->a1, EOBPars->a2, EOBPars->aK2, C_Q1, C_Q2, C_Oct1, C_Oct2, C_Hex1, C_Hex2, usetidal, &rc_vec[i], &drc_dr_vec[i], &pl_hold);
        eob_dyn_s_GS(dyn->r, rc_vec[i], drc_dr_vec[i], EOBPars->aK2, dyn->prstar, dyn->pphi, nu, chi1, chi2, X1, X2, c3, ggm);
      ...
      //printf("r, omg, chis =%.10f %.10f %.10f %.10f\n", dyn->r, dyn->Omg, chi1, chi2);      
      if (parity)  {
      ...
      * ********************************** */
      ...
      //FIXME Non-spinning routine gives 1e-2 difference between PA and full EOB waveform. Tested cases: bbh q 1 f 0.001 and q 5 f 0.006.
        ...
        */
        ...
        /** New GGM functions */
        eob_dyn_s_GS(dyn->r, rc_vec[i], drc_dr_vec[i], EOBPars->aK2, dyn->prstar, 0.0, nu, chi1, chi2, X1, X2, c3, ggm);
