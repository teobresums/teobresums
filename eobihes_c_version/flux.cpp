...
double flux(const double x, const double Omega, const double r_omega, const double E, const double Heff, const double jhat, const double r, const double prstar, const double ddotr, double source[], void *params)
{
    ...
    vector<gsl_complex> hlm_NQC = hlmNQC(nu,r,prstar,Omega,ddotr); /** Compute NQC correction to the modulus of the (l,m) waveform */

    for (int k=35; k--;)
    {
        Modhhatlm = source[k] * MTlm[k] * flm[k]; /** Compute modulus of hhat_lm */
        
        if (NQC_flag==true) /** NQCs */
        {
            switch (k)
            {
                case 1:
        ...
        if (tidal_flag==true) /** Tidal corrections */
        {
            switch (k)
            {
                case 0: /** (2,1)  */
    ...
    Fphi = -32.0/5.0 * nu * gsl_pow_int(r_omega,4) * gsl_pow_int(Omega,5) * hatf;
