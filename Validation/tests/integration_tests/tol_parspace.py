...
try:
    ...
    print("Warning! PyART not installed; not computing mismatch.")
    pyart = False
...
def run_tols(delta_log_tol, pardic, f0):
    ...
    try:
        ...
        # Merger index on default wf
        jmrg = np.argmax(hlm0['1'][0])
        ...
        for tol_dict in change_tols:
            ...
            dphis.append(np.interp(t0[jmrg], t, np.unwrap(hlm['1'][1])) - np.unwrap(hlm0['1'][1])[jmrg])
...
if __name__=='__main__':
    ...
    parser.add_argument('--f0',         type=float,          default=10.,            help='Initial frequency [Hz].')
    parser.add_argument('--chi',        type=float, nargs=2, default=[-0.99, 0.99],  help='Spin range for the binary (spin aligned, z-component).')
    ...
    parser.add_argument('--worst',      action='store_true',                         help='Plot the worst matching wf according to the given pc variable')
    ...
    if args.plot:
        ...
        if args.worst:
            ...
            if pyart:
                deltaphi = np.interp(t0, t, np.unwrap(hlm['1'][1])) - np.unwrap(hlm0['1'][1])
